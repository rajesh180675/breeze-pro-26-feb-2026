...
from app.application.option_chain.service import fetch_option_chain_snapshot, load_replay_frame, merge_live_overlay
from app.domain.option_chain.workspace import resolve_monitored_strike_defaults
...
def render_option_chain_controls(
    ...
    with c3:
        ...
        compare_expiries = st.multiselect("Compare Expiries", options=[exp for exp in expiries if exp != expiry], default=compare_default[:2], format_func=format_expiry, key="oc_compare_expiries")[:2]
    ...
    with c4:
        ...
        n_strikes = st.slider("Strikes ±", 5, 100, 40, key="oc_n", disabled=show_all, help="Number of strikes to show above and below ATM. Ignored when 'Show All Strikes' is checked.")
    with c5:
    ...
    with st.sidebar.expander("⛓️ Chain Options"):
        ...
        change_window = st.selectbox("Change Window", ["Since Open", "5m", "15m", "30m", "60m"], index=["Since Open", "5m", "15m", "30m", "60m"].index(state.get("change_window", "Since Open")), key="oc_change_window")
        normalization_mode = st.selectbox("Compare Normalization", ["Absolute", "ATM Offset", "ATM %"], index=["Absolute", "ATM Offset", "ATM %"].index(str(state.get("normalization_mode", "Absolute")).title() if str(state.get("normalization_mode", "Absolute")).lower() != "atm %" else "ATM %"), key="oc_normalization_mode")
        chart_tab = st.selectbox("Chart Tab", ["OI Profile", "Delta OI", "IV Smile", "Compare OI", "Compare IV Smile", "Term Structure", "Expected Move", "OI Heatmap", "Skew Replay", "Gamma", "Vanna", "Charm", "Liquidity"], key="oc_chart_tab")

    return OptionChainControls(inst=inst, cfg=cfg, expiry=expiry, compare_expiries=compare_expiries, show_all=show_all, n_strikes=n_strikes, refresh=refresh, show_greeks=show_greeks, view=view, quote_mode=quote_mode, chain_opt_oi_heatmap=chain_opt_oi_heatmap, chain_opt_volume_spike=chain_opt_volume_spike, chain_opt_pcr_gauge=chain_opt_pcr_gauge, chain_opt_max_pain_marker=chain_opt_max_pain_marker, chain_opt_iv_percentile=chain_opt_iv_percentile, chain_opt_export=chain_opt_export, sticky_atm=sticky_atm, show_only_liquid=show_only_liquid, show_only_unusual=show_only_unusual, change_window=change_window, normalization_mode=normalization_mode, chart_tab=chart_tab)
...
def load_cached_option_chain(cache_manager: Any, client: Any, db: Any, instrument: str, cfg: Any, expiry: str, spinner: Callable[[str], Any]) -> pd.DataFrame:
...
def load_compare_option_frames(loader: Callable[[str], pd.DataFrame], expiry: str, compare_expiries: Sequence[str]) -> Dict[str, pd.DataFrame]:
...
def resolve_replay_chain_selection(db: Any, session_state: Dict[str, Any], instrument: str, expiry: str, quote_mode: str, update_state: Callable[..., Dict[str, Any]], caption: Callable[[str], None], info: Callable[[str], None]) -> Tuple[List[str], str, pd.DataFrame]:
...
def sync_option_chain_watchlist(db: Any, session_state: Dict[str, Any], state: Dict[str, Any], instrument: str, expiry: str, all_strikes: Sequence[int]) -> List[int]:
    ...
    monitor_strikes = st.sidebar.multiselect("Monitor Strikes", options=list(all_strikes), default=default_monitored, key="oc_monitored_strikes")
    ...
    session_state["option_chain_workspace_state"] = {**session_state.get("option_chain_workspace_state", {}), "monitored_strikes": normalized}
    return normalized


def sync_option_chain_live_feed(live_feed_module: Any, subscribe_fn: Callable[[str, str, List[int], Any], Dict[str, str]], session_state: Dict[str, Any], instrument: str, expiry: str, api_code: str, visible_strikes: Iterable[int], client: Any) -> Dict[str, str]:
...
def apply_option_chain_live_overlay(quote_mode: str, display_df: pd.DataFrame, token_map: Dict[str, str], instrument: str, expiry: str) -> pd.DataFrame:
