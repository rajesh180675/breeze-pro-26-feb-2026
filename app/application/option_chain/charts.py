...
def build_oi_profile_figure(df: pd.DataFrame, atm: float = 0.0, max_pain: float = 0.0, selected_strike: float = 0.0) -> go.Figure:
    ...
    for right, color in (("Call", "#d62728"), ("Put", "#2ca02c")):
        ...
        fig.add_trace(go.Bar(x=side["strike_price"], y=side["open_interest"], name=f"{right} OI", marker_color=color))
    if atm:
...
def build_replay_delta_oi_figure(
    ...
    for option_type, label, color in (("CE", "Call ΔOI", "#ff9896"), ("PE", "Put ΔOI", "#98df8a")):
        ...
        fig.add_trace(go.Bar(x=side["strike"], y=side["open_interest_change"], name=label, marker_color=color, customdata=side["strike"]))
    if atm:
        _marker_annotation(fig, atm, "ATM", "#1f77b4")
    _selected_marker(fig, selected_strike)
    _replay_timestamp_annotation(fig, replay_timestamp)
    fig.update_layout(title=f"Replay Delta/OI Change ({change_window})", barmode="group", hovermode="x unified", yaxis_title="Open Interest Change")
    return fig
...
def build_term_structure_figure(expiry_summaries: Iterable[Dict[str, float]]) -> go.Figure:
    ...
    fig = go.Figure(go.Scatter(x=[row["expiry"] for row in summaries], y=[row["atm_iv"] * 100 for row in summaries], mode="lines+markers", name="ATM IV"))
...
def build_expected_move_figure(expiry_summaries: Iterable[Dict[str, float]], spot: float) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=[row["expiry"] for row in summaries], y=[row["expected_move"] for row in summaries], name="Expected Move", marker_color="#1f77b4"))
    if spot > 0:
        fig.add_annotation(x=0, y=1.05, yref="paper", text=f"Spot {spot:,.0f}", showarrow=False)
...
def build_oi_heatmap_figure(snapshot_df: pd.DataFrame, replay_timestamp: str = "") -> go.Figure:
    ...
    fig = go.Figure(go.Heatmap(x=list(pivot.columns), y=list(pivot.index), z=pivot.values, colorscale="Viridis", colorbar={"title": "OI"}))
...
def build_skew_shift_replay_figure(snapshot_df: pd.DataFrame, replay_timestamp: str = "") -> go.Figure:
    ...
    grouped = snapshot_df.groupby(["snapshot_ts", "option_type"], dropna=True)["iv"].mean().unstack(fill_value=0).reset_index().sort_values("snapshot_ts")
    grouped["skew_shift"] = grouped.get("PE", 0) - grouped.get("CE", 0)
    fig = go.Figure(go.Scatter(x=grouped["snapshot_ts"], y=grouped["skew_shift"], mode="lines+markers", name="PE-CE Skew"))
...
def build_liquidity_scatter_figure(df: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure(go.Scatter(x=df["spread_pct"], y=df["volume"], mode="markers", text=df["strike_price"], marker={"size": (df["open_interest"].fillna(0) / 1000.0).clip(lower=8, upper=30)}, name="Strikes"))
