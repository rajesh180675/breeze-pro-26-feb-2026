...
def build_option_chain_chart(
    ...
    gamma_walls: Sequence[Dict[str, Any]],
):
    ...
    if chart_tab == "Delta OI":
        return build_replay_delta_oi_figure(
            change_df,
            atm=atm,
...
def render_option_chain_display(
    view: str,
    display_df: pd.DataFrame,
    ladder: pd.DataFrame,
    selected_strike: Optional[int],
    pinned_strikes: Sequence[int],
    atm: float,
    max_pain: float,
    ...
    if view == "Ladder":
        ...
        ladder_selection = st.dataframe(
            style_option_chain_rows(ladder, selected_strike, pinned_strikes=pinned_strikes, atm=atm, max_pain=max_pain),
            height=640,
            hide_index=True,
            width="stretch",
            key="oc_ladder_table",
    ...
    if view == "Calls Only":
        st.dataframe(
            style_option_chain_rows(display_df[display_df["right"] == "Call"].copy(), selected_strike, pinned_strikes=pinned_strikes, atm=atm, max_pain=max_pain),
            height=640,
            hide_index=True,
            width="stretch",
        )
    ...
    if view == "Puts Only":
        st.dataframe(
            style_option_chain_rows(display_df[display_df["right"] == "Put"].copy(), selected_strike, pinned_strikes=pinned_strikes, atm=atm, max_pain=max_pain),
            height=640,
            hide_index=True,
            width="stretch",
        )
        return selected_strike
    st.dataframe(
        style_option_chain_rows(display_df, selected_strike, pinned_strikes=pinned_strikes, atm=atm, max_pain=max_pain),
        height=640,
        hide_index=True,
        width="stretch",
    )
...
def render_option_chain_analysis(
    panel_payload: Dict[str, Any],
    change_df: pd.DataFrame,
    top_movers: Dict[str, pd.DataFrame],
...
def render_option_chain_chart(
    ...
    if fig is None:
        return selected_strike
    plotly_selection = st.plotly_chart(
        fig,
